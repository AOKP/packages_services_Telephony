...
public class Ringer {
    ...
    void ring() {
        ...
        synchronized (this) {
            ...
            if (mFirstRingEventTime < 0) {
                ...
                boolean increasing = Settings.AOKP.getInt(cr,
                        Settings.AOKP.INCREASING_RING, 0) == 1;
                int minVolume = Settings.AOKP.getInt(cr,
                        Settings.AOKP.INCREASING_RING_MIN_VOLUME, 1);
                if (increasing && minVolume < ringerVolume) {
                    mRingIncreaseInterval = Settings.AOKP.getInt(cr,
                            Settings.AOKP.INCREASING_RING_INTERVAL, 0);
    ...
    private boolean inQuietHours() {
        return QuietHoursHelper.inQuietHours(mContext, Settings.AOKP.QUIET_HOURS_RINGER);
    }
}
